"""create_functional_tables

Revision ID: cc7d724c7dd8
Revises: ffdc0a98111c
Create Date: 2024-07-22 15:26:19.263736
...
def upgrade():
    ...
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('ownerId', sa.Integer(), nullable=False),
    sa.Column('status', sa.Enum('pending', 'processing', 'delivery', 'delivered', name='packagestatus'), nullable=False),
    sa.Column('time_created', sa.DateTime(timezone=True), server_default=sa.text('(CURRENT_TIMESTAMP)'), nullable=True),
    sa.Column('time_updated', sa.DateTime(timezone=True), server_default=sa.text('(CURRENT_TIMESTAMP)'), nullable=True),
    sa.ForeignKeyConstraint(['ownerId'], ['users.id'], ),
    sa.PrimaryKeyConstraint('id')
    ...
    if environment == "production":
        op.execute(f"ALTER TABLE transaction_details SET SCHEMA {SCHEMA};")
